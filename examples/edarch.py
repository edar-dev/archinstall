...
if archinstall.arguments.get('help', None):
	...
	archinstall.log(f" - optional root password via --rootpw=<password>")
	archinstall.log(f" - optional user name via --username=<username>")
	archinstall.log(f" - optional user password via --userpw=<password>")
...
def install_on(mountpoint):
	...
	with archinstall.Installer(mountpoint) as installation:
		...
		if installation.minimal_installation():
			...
			installation.add_additional_packages(['nano', 'wget', 'git', 'vim'])
			installation.install_profile('minimal')

			installation.user_create(archinstall.arguments.get('username', 'devel'), archinstall.arguments.get('userpw', 'devel'))
			installation.user_set_pw('root', archinstall.arguments.get('rootpw', 'airoot'))

	# Once this is done, we output some useful information to the user
	# And the installation is complete.
	archinstall.log(f"There are two new accounts in your installation after reboot:")
	archinstall.log(f" * root (password: {archinstall.arguments.get('rootpw', 'airoot')})")
	archinstall.log(f" * {archinstall.arguments.get('username', 'devel')} (password: {archinstall.arguments.get('userpw', 'devel')})")
